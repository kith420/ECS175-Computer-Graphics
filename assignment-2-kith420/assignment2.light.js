...
class AmbientLight extends Light {
     ...
     */
    update( ) {
        // ambient light gizmo is a cube (box_scale [10000.0, 10000.0, 10000.0])
        this.target_shader.use()
        this.target_shader.setUniform3f(`u_lights_ambient[${this.id}].color`, this.color) // light color
        this.target_shader.setUniform1f(`u_lights_ambient[${this.id}].intensity`, this.intensity) // light intensity
        this.target_shader.unuse() // unuse the shader
    }
}
...
class DirectionalLight extends Light {
     ...
     */
    update( ) {
        // directional light gizmo is elongated along the Y axis (box_scale [0.025, 0.25, 0.025])
        // direction is the local Y axis transformed by the model matrix
        // @ column-major layout (glmatrix), the Y axis is at indices [4], [5], [6]
...
class PointLight extends Light {
     ...
     */
    update( ) {
        // position = translation column of the model matrix
        // In column-major layout (glmatrix), translation is at indices [12], [13], [14]
        ...
        // use the target shader
        this.target_shader.use()
        this.target_shader.setUniform3f(`u_lights_point[${this.id}].color`, this.color) // light color
        this.target_shader.setUniform1f(`u_lights_point[${this.id}].intensity`, this.intensity) // light intensity
        this.target_shader.setUniform3f(`u_lights_point[${this.id}].position`, position) // light position
        this.target_shader.unuse() // unuse the shader
    }
}
