...
class Object3D
{
    ...
    createVAO( gl, shader )
    {
        ...
        let stride = this.num_components * 4; // 4 bytes per float

        // set up position attribute (a_position)
        ...
        // set up normal attribute (a_normal)
        // only done if we have interleaved normals
        if (this.num_components >= 6) {
            let a_normal = shader.getAttributeLocation('a_normal');
            if (a_normal >= 0) {
                gl.enableVertexAttribArray(a_normal);
                gl.vertexAttribPointer(a_normal, 3, gl.FLOAT, false, stride, 3 * 4); // offset past position (3 * 4 bytes per float)
            }
        }
...
class ShadedObject3D extends Object3D { 
     ...
     */
     constructor( material, gl, shader, vertices, indices, draw_mode ) {
        ...
        // the vertex data for shaded objects is interleaved: [pos, normal, pos, normal, ...]
        // each vertex has 6 components (3 position & 3 normal)
        // update num_components and recreate the VAO to match interleaved layout
        this.num_components = 6
        gl.deleteVertexArray(this.vertex_array_object)
        this.createVAO(gl, shader)
     }
