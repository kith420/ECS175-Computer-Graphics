...
class ShadedObject3D extends Object3D { 
    ...
    createVAO( gl, shader )
    {
        ...
        // 3 pos + 3 norm = 6, add 3 tangent + 2 texcoord = 11 if textured
        let num_total_components = 6
        num_total_components += this.material.hasTexture() ? 5 : 0

        // stride = bytes between consecutive vertices in the interleaved buffer
        stride = num_total_components * Float32Array.BYTES_PER_ELEMENT

        let location = shader.getAttributeLocation( 'a_position' )
        if (location >= 0) {
            offset = 0 // position is first
            gl.enableVertexAttribArray( location )
            gl.vertexAttribPointer( location, this.num_components_vec3, gl.FLOAT, false, stride, offset )
        }
        ...
        location = shader.getAttributeLocation( 'a_normal' )
        if (location >= 0) {
            offset = this.num_components_vec3 * Float32Array.BYTES_PER_ELEMENT // after position (12 bytes)
            gl.enableVertexAttribArray( location )
            gl.vertexAttribPointer( location, this.num_components_vec3, gl.FLOAT, false, stride, offset )
        }
        ...
        if (location >= 0 && this.material.hasTexture()) {
            offset = 2 * this.num_components_vec3 * Float32Array.BYTES_PER_ELEMENT // after pos + norm (24 bytes)
            gl.enableVertexAttribArray( location )
            gl.vertexAttribPointer( location, this.num_components_vec3, gl.FLOAT, false, stride, offset )
        }
        ...
        if (location >= 0 && this.material.hasTexture()) {
            offset = 3 * this.num_components_vec3 * Float32Array.BYTES_PER_ELEMENT // after pos + norm + tangent (36 bytes)
            gl.enableVertexAttribArray( location )
    ...
    render( gl )
    {
        ...
        // map each sampler to a texture unit
