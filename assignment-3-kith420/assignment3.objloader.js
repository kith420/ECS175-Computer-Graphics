...
class OBJLoader {
    ...
    load( gl ) {
        ...
        // no texture:   [pos, norm, pos, norm, ...]             -> 6 floats/vertex
        // with texture:  [pos, norm, tangent, uv, ...]          -> 11 floats/vertex
        let vertex_data = []
        let num_vertices = vertex_positions.length/3
        ...
        if (material.hasTexture()) {
            ...
            for (let i = 0; i < num_vertices; i++) {
                vertex_data.push(vertex_positions[i * 3], vertex_positions[i * 3 + 1], vertex_positions[i * 3 + 2])       // position
                vertex_data.push(vertex_normals[i * 3], vertex_normals[i * 3 + 1], vertex_normals[i * 3 + 2])             // normal
                vertex_data.push(vertex_tangents[i * 3], vertex_tangents[i * 3 + 1], vertex_tangents[i * 3 + 2])          // tangent
                vertex_data.push(vertex_texture_coords[i * 2], vertex_texture_coords[i * 2 + 1])                          // texcoord
            }
        } else {
            for (let i = 0; i < num_vertices; i++) {
                vertex_data.push(vertex_positions[i * 3], vertex_positions[i * 3 + 1], vertex_positions[i * 3 + 2])       // position
