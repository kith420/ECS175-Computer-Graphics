...
class DirectionalLight extends Light {
    ...
    getCamera( scale )
    {
        ...
        // get the light direction from the model matrix rotation applied to the default down vector
        let transform = mat4.getRotation(mat4.create(), this.model_matrix)
        let dir = vec3.normalize(vec3.create(), vec3.transformQuat(vec3.create(), [0.0, -1.0, 0.0], transform))

        // place the light camera behind the scene along the opposite of the light direction
        let eye = vec3.scale(vec3.create(), vec3.negate(vec3.create(), dir), radius)
        let center = vec3.add(vec3.create(), eye, dir)

        let cam = new OrthoCamera(eye, center, -radius, radius, -radius, radius, 0.01, radius * 4)
        return cam
    }
}
...
class PointLight extends Light {
    ...
    getCamera( scale )
    {
        ...
        // always look toward the origin of the scene
        ...
        let radius = vec3.length(scale)
        let cam = new PerspectiveCamera(eye, center, fovy, 1, 0.01, radius * 4)
        return cam
    }
}
