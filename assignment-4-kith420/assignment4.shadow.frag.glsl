...
{
    // perspective divide to get texture-space coords
    ...
    // sample the stored depth from the shadow map
    float shadow_map_depth = texture(shadow_tex, proj_coords.xy).r;

    float current_depth = proj_coords.z;

    // small constant bias to avoid shadow acne
    float bias = 0.005;

    bool visible = (current_depth - bias) <= shadow_map_depth;
    if (visible) {
        return 1.0;
    }
    else {
...
void main() {
    ...
    // visualize normals only if the flag is set
    if (u_show_normals) {
