...
class SceneNode {
    ...
    calculateWorldTransformation() {
        let hierarchy = this.getTransformationHierarchy([])
        // If there are no transformations, return an identity matrix
        if (hierarchy.length === 0) return mat4.create()
        let world = mat4.clone(hierarchy[hierarchy.length - 1])
        let temp = mat4.create()
        for (let i = hierarchy.length - 2; i >= 0; i--) {
            mat4.multiply(temp, world, hierarchy[i])
            mat4.copy(world, temp)
        }
        return world
    }
